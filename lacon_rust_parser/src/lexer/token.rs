...
impl Token {
    pub fn get_unit_multiplier(&self) -> f64 {
        let suffix = self.get_unit_suffix();

        // 1. Находим UnitDef, чтобы знать группы префиксов и базовые части
        let origin = self.get_unit_origin_suffix();
        ...
        // 2. Считаем множители префиксов
        // Внутри get_unit_multiplier для составных юнитов:
        if let Some((base_num, base_den)) = def.parts {
            let parts: Vec<&str> = suffix.split('/').collect();
            if parts.len() == 2 {
                // Для числителя степень обычно 1.0, для знаменателя берем из пропсов
                let m_num =
                    self.extract_prefix_value(parts[0], base_num, &def.numerator_group, 1.0);
                let m_den = self.extract_prefix_value(
                    parts[1],
                    base_den,
                    &def.denominator_group,
                    def.props.exponent,
                );
                return m_num / m_den;
            }
        ...
        // Для атомарных
        self.extract_prefix_value(suffix, def.symbol, &def.numerator_group, def.props.exponent)
    }

    fn extract_prefix_value(
        &self,
        current: &str,
        base: &str,
        group: &PrefixGroup,
        ...
        if let Some(index) = current.find(base) {
            ...
            let prefix_multiplier = PREFIXES
                .iter()
                .find(|(p_str, _, p_group)| p_group == group && *p_str == potential_prefix)
                .map(|(_, val, _)| *val)
                .unwrap_or(1.0);

            // Если есть приставка, возводим её в степень, прописанную в UnitDef
            return prefix_multiplier.powf(exponent);
        }
    ...
    pub fn get_normalized_value(&self) -> f64 {
        let val: f64 = self
            ...
            .parse()
            .unwrap_or(0.0);
        let multiplier = self.get_unit_multiplier();

        // Находим UnitDef для применения scale/offset
        let origin = self.get_unit_origin_suffix();
        let unit_def = crate::shared::unit::units::UNITS.iter().find(|u| {
            u.symbol == origin
                || u.parts
                    .as_ref()
                    .map_or(false, |(n, d)| origin == format!("{}/{}", n, d))
        ...
        if let Some(def) = unit_def {
            // Сначала применяем префиксы, потом нормализацию UnitDef (scale/offset)
            def.normalize(val * multiplier)
        } else {
            val * multiplier
        }
    }
